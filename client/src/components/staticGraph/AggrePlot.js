...
class AggrePlot extends Component {
  ...
  updateD3() {
    if (
      this.props.impairedData.length > 1 &&
      this.props.unImpairedData.length > 1
    ) {
      ...
      this.xScale
        .domain(d3.extent(unImpairedData, d => d.date))
        .range([0, width]);

      this.yScale
        .domain(d3.extent(unImpairedData, d => d.discharge))
        .range([height, 0]);
